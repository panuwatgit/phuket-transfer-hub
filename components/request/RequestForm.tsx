              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {VEHICLE_ORDER.map((k) => (
                  <button type="button" key={k} className={`choice ${s.vehicleType === k ? "on" : ""}`} onClick={() => set("vehicleType", k)}>
                    {VEHICLE_IMAGE[k]
                      ? <div className="h-14 mb-1 grid place-items-center"><Image src={VEHICLE_IMAGE[k]!.src} alt={t.vehicle[k].name} width={VEHICLE_IMAGE[k]!.w} height={VEHICLE_IMAGE[k]!.h} sizes="160px" className="h-[52px] w-auto drop-shadow-[0_8px_10px_rgba(26,43,60,.2)]" /></div>
